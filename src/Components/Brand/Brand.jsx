...
export const Brand = ({ onFetchBrand, brand }) => {

    const [pages, setpages] = useState(1)
    const [filter, setfilter] = useState("")


    function filterBrand(list, value) {
        if (value === null || value === undefined) return list
        else {
            list = list.filter(params => {
                //params.MA_HANG.toLowerCase().indexOf(value.toLowerCase()) !== -1||
                return  params.brandName.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").indexOf(value.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")) !== -1
    ...
    useEffect(() => {
        // phải dung asyn vì nó sẽ fetch ra lise line Brand k kịp dẫn tới lấy state cũ 
        //vì state mới chưa kịp thay đổi do dang trong thời gian fetch
        async function fectchBrand() {
            await onFetchBrand()
        }
        fectchBrand()
    ...
    brand = filterBrand(brand, filter)

    var BrandList = brand.map((linepro, index) => {
        if (index >= (pages - 1) * 10 && index < pages * 10) {
            return <BrandItem item={linepro} key={index} />
        }
        return null
    })
            ...
            <div className="card shadow mb-4">
                ...
                <div className="card-body">
                    <div className="table-responsive">
                        ...
                        <NavigationSwitchPage entries={brand.length} onReceivePage={p => setpages(p)} />
                    </div>
                </div>
            </div>
        </div >
